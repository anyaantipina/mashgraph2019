...
ShaderProgram::ShaderProgram(const std::unordered_map<GLenum, std::string> &inputShaders)
{
  ...
  shaderProgram = glCreateProgram(); GL_CHECK_ERRORS

  if (inputShaders.find(GL_VERTEX_SHADER) != inputShaders.end())
  {
    shaderObjects[GL_VERTEX_SHADER] = LoadShaderObject(GL_VERTEX_SHADER, inputShaders.at(GL_VERTEX_SHADER)); GL_CHECK_ERRORS
    glAttachShader(shaderProgram, shaderObjects[GL_VERTEX_SHADER]); GL_CHECK_ERRORS
  }

  if (inputShaders.find(GL_FRAGMENT_SHADER) != inputShaders.end())
  {
    shaderObjects[GL_FRAGMENT_SHADER] = LoadShaderObject(GL_FRAGMENT_SHADER, inputShaders.at(GL_FRAGMENT_SHADER)); GL_CHECK_ERRORS
    glAttachShader(shaderProgram, shaderObjects[GL_FRAGMENT_SHADER]); GL_CHECK_ERRORS
  }
  ...
  glLinkProgram(shaderProgram); GL_CHECK_ERRORS

  GLint linkStatus;


  glGetProgramiv(shaderProgram, GL_LINK_STATUS, &linkStatus); GL_CHECK_ERRORS
...
void ShaderProgram::SetUniform(const std::string &location, float x, float y, float z, float w) const
{
    GLint uniformLocation = glGetUniformLocation(shaderProgram, location.c_str());
    if (uniformLocation == -1)
    {
...
void ShaderProgram::SetUniform(const std::string &location, float r, float g, float b) const
{
    GLint uniformLocation = glGetUniformLocation(shaderProgram, location.c_str());
    if (uniformLocation == -1)
    {
